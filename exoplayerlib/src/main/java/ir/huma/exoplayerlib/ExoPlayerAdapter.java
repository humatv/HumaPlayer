...
public class ExoPlayerAdapter extends PlayerAdapter implements Player.EventListener {
    ...
    public void fastForward() {
        if (getDuration() > -1) {
            long newPosition = getCurrentPosition() + 10*1000;
     ...
     */
    public boolean setDataSource(Uri uri, boolean isLive) {
        if (uri == null) {
            return false;
        }
        mMediaSourceUri = uri;
        prepareMediaForPlaying(isLive);
        return true;
    }
     ...
     */
    public MediaSource onCreateMediaSource(Uri uri, boolean isLive) {
    ...
    private void prepareMediaForPlaying(boolean isLive) {
        reset();
        if (mMediaSourceUri != null) {
            MediaSource mediaSource = onCreateMediaSource(mMediaSourceUri, isLive);
